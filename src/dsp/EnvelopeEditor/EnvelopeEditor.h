...
#include "juce_shim.h"   /* SCHWUNG PORT: JUCE-free Array / juce::Point / Timer / locks */
...
public:
    ...
    /* SCHWUNG PORT: sync(float, AudioPlayHead::CurrentPositionInfo) and
     * setTimeInformation(AudioPlayHead::CurrentPositionInfo) removed — JUCE
     * host-transport overloads. The float overload below is used instead
     * (host BPM passed directly via SynthEngine::setEnvelopeEditorBpm). */
